<template>
  <div style='width: 100%'>
    <div class="tableTop">
      ...
      <a-space :size="20">
        <span style="color: #3cb371">启用： 4</span>
        <span style="color: #ff0000">停用： 2</span>
        <a-input-search
    ...
    >
      <template v-slot:name="text, record">
        <a-button @click="targetClick(record,'detail')" type="link">{{ text }}</a-button>
      </template>
      <template v-slot:operation="text, record">
        <a-button @click="targetClick(record,'update')" type="link">修改</a-button>
      </template>
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="8">
            <a-form-model-item label="适用对象" prop="applicableObjects">
              <a-input :disabled="formDisabled" v-model="form.applicableObjects" />
            </a-form-model-item>
          </a-col>
          <a-col :span="8">
...
export default {
  ...
  data() {
    return {
      ...
      columns: table.columns.targetMetaData,
      tableLoading: false,
      // modal框
      visible: false,
      modalTitle: "",
